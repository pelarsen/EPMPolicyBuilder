...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      bullet("Developer Mode enabled (Settings > System > For developers) — required to deploy and run packaged WinUI 3 apps locally."),
      bullet(".NET 10 SDK if building from source (the app targets net10.0-windows10.0.26100.0)."),
      ...
      p("EPM Policy Builder can be run in two ways: from source using the included build script, or as a packaged app after publishing an MSIX."),
      h2("3.1 Running from source"),
      p("From the project root (" ),
      code("C:\\Users\\PerLarsen\\OneDrive - Osddeployment\\EPMPolicyBuilder"),
      p("run the bundled build helper:"),
      code(".\\BuildAndRun.ps1 EPMPolicyBuilder.csproj"),
      p("The script:"),
      numbered("Confirms Developer Mode is enabled (fails fast with instructions if not)."),
      numbered("Auto-detects the platform (x64 or ARM64) and defaults to a Debug build."),
      numbered("Locates MSBuild via vswhere, falling back to \u201cdotnet build\u201d if Visual Studio is not installed."),
      numbered("Launches the built app using the winapp CLI once the build succeeds."),
      p("Useful variations:"),
      bullet("Build only, without launching: .\\BuildAndRun.ps1 EPMPolicyBuilder.csproj -SkipRun"),
      bullet("Launch detached (background): .\\BuildAndRun.ps1 EPMPolicyBuilder.csproj -Detach"),
      bullet("Override configuration: .\\BuildAndRun.ps1 EPMPolicyBuilder.csproj /p:Configuration=Release"),
      h2("3.2 Running a packaged build"),
      p("For distribution to other administrators, publish the project as an MSIX package (Solution Explorer > Package and Publish, or dotnet publish with the appropriate publish profile) and install it on the target machine. Developer Mode or a trusted signing certificate is required to install a self-signed MSIX outside the Microsoft Store."),
